<div class="row">
        <div class="content-wrapper-before theme_header_page_bg"></div>
        <div class="breadcrumbs-dark pb-0" id="breadcrumbs-wrapper">
          <div class="container">
            <div class="row">
              <div class="col s10 m6 l6">
                <h5 class="breadcrumbs-title mt-0 mb-0"><span>Company Registration</span></h5>
                <ol class="breadcrumbs mb-0">
                  <li class="breadcrumb-item"><a href="index.html">Home</a>
                  </li>
                  <li class="breadcrumb-item"><a href="#">Form</a>
                  </li>
                  <li class="breadcrumb-item active">Form Layouts
                  </li>
                </ol>
              </div>
              <div class="col s2 m6 l6"><a class="btn dropdown-settings waves-effect waves-light breadcrumbs-btn right" href="#!" data-target="dropdown1"><i class="material-icons hide-on-med-and-up">settings</i><span class="hide-on-small-onl">Options</span><i class="material-icons right">arrow_drop_down</i></a>
                <ul class="dropdown-content" id="dropdown1" tabindex="0">
                  <li tabindex="0"><a class="grey-text text-darken-2" href="user-profile-page.html">List</a></li>
                </ul>
              </div>
            </div>
          </div>
        </div> <!-- End Of #breadcrumbs-wrapper -->

        <div class="col s12">
          <div class="container">
            <div class="seaction">
              <!-- <div class="card">
                <div class="card-content">
                  <p class="caption mb-0">Register your company profile.</p>
                </div>
              </div> -->
              <!-- End of card-->

              <div class="row">
                <form class="theme_form_type1">
                    <div class="col s12 m12 l12">
                      <div class="card card card-default scrollspy">
                        <div class="card-content">
                          <h4 class="card-title">Project Info</h4>
                              <div class="row">
                                <div class="input-field col m4 l4 s12 theme_select2_style1">
                                    <select class="select2-theme browser-default">
                                      <option value="">Select Company</option>
                                      <option value="COMP1">Company 1</option>1
                                    </select>
                                </div>
                                <div class="input-field col m4 l4 s12">
                                  <input id="cmp_name" name="cmp_name" type="text" class="border-full-radius medium-height box-shadow-none" placeholder="" />
                                  <label for="cmp_name" class="floating-border-lbl">Project Name</label>
                                </div>
                                <div class="input-field col m4 l4 s12">
                                  <input id="cmp_name" name="cmp_name" type="text" class="border-full-radius medium-height box-shadow-none" placeholder="" />
                                  <label for="cmp_name" class="floating-border-lbl">Project Start Date</label>
                                </div>
                                
                              </div> <!-- End of row-->
                              <div class="row">
                                 <div class="input-field col m4 l4 s12">
                                  <input id="cmp_email" name="cmp_email" type="email" class="border-full-radius medium-height box-shadow-none" placeholder="" />
                                  <label for="cmp_email" class="floating-border-lbl">Project End Date (Approx)</label>
                                </div>
                                  <div class="input-field col m4 l4 s12">
                                    <textarea id="address" name="address"  class="materialize-textarea border-full-radius box-shadow-none" placeholder="" rows="10" cols="50" /></textarea>
                                    <label for="address" class="floating-border-lbl">Address</label>
                                  </div>
                                  <div class="input-field col m4 l4 s12 theme_select2_style1">
                                    <select class="select2-theme browser-default">
                                      <option value="">Choose State</option>
                                      <option value="WB">West Bengal</option>
                                    </select>
                                  </div>
                              </div> <!-- End of row-->
                              <!-- <div class="row">
                                  <div class="input-field col m4 l4 s12">
                                    <input id="city" name="city" type="text" class="border-full-radius medium-height box-shadow-none" placeholder="" />
                                    <label for="city" class="floating-border-lbl">Contact Person Name</label>
                                  </div>
                                  <div class="input-field col m4 l4 s12">
                                    <input id="city" name="city" type="text" class="border-full-radius medium-height box-shadow-none" placeholder="" />
                                    <label for="city" class="floating-border-lbl">Contact Person Number</label>
                                  </div>
                                  <div class="input-field col m4 l4 s12">
                                    <textarea id="address" name="address"  class="materialize-textarea border-full-radius box-shadow-none" placeholder="" rows="10" cols="50" /></textarea>
                                    <label for="address" class="floating-border-lbl">Contact Person Address</label>
                                  </div>
                              </div> End of row -->
                        </div>
                      </div>
                  </div> <!-- End of Add Project -->

                  <div class="col s12 m12 l12">
                      <div class="card card card-default scrollspy">
                        <div class="card-content">
                          <h4 class="card-title">Payment</h4>
                              <div class="row">
                                <div class="input-field col m3 l3 s12 theme_select2_style1">
                                    <select class="select2-theme browser-default">
                                      <option value="">Choose Rate</option>
                                      <option value="RATE1">6 Months - 2500/-</option>
                                    </select>
                                </div>
                                <div class="input-field col m3 l3 s12">
                                  <input id="cmp_name" name="cmp_name" type="text" class="border-full-radius medium-height box-shadow-none" placeholder="" />
                                  <label for="cmp_name" class="floating-border-lbl">Payment Date</label>
                                </div>
                                <div class="input-field col m3 l3 s12">
                                  <input id="cmp_email" name="cmp_email" type="email" class="border-full-radius medium-height box-shadow-none" placeholder="" />
                                  <label for="cmp_email" class="floating-border-lbl">Licence Validity</label>
                                </div>
                                <div class="input-field col m3 l3 s12 theme_select2_style1">
                                    <select class="select2-theme browser-default">
                                      <option value="">Payment For</option>
                                      <option value="P">Purchase</option>
                                      <option value="R">Renewal</option>
                                    </select>
                                  </div>
                              </div> <!-- End of row-->
                              <div class="row">
                                  <div class="input-field col m3 l3 s12">
                                    <input id="city" name="city" type="text" class="border-full-radius medium-height box-shadow-none" placeholder="" />
                                    <label for="city" class="floating-border-lbl">Subscription Amount</label>
                                  </div>
                                  <div class="input-field col m3 l3 s12">
                                    <input id="city" name="city" type="text" class="border-full-radius medium-height box-shadow-none" placeholder="" />
                                    <label for="city" class="floating-border-lbl">Offer Discount Rate</label>
                                  </div>
                                  <div class="input-field col m3 l3 s12">
                                    <input id="city" name="city" type="text" class="border-full-radius medium-height box-shadow-none" placeholder="" />
                                    <label for="city" class="floating-border-lbl">Offer Discount Rate</label>
                                  </div>
                                  <div class="input-field col m3 l3 s12">
                                    <input id="city" name="city" type="text" class="border-full-radius medium-height box-shadow-none" placeholder="" />
                                    <label for="city" class="floating-border-lbl">Extraa Discount Amount</label>
                                  </div>
                              </div> <!-- End of row-->
                              <div class="row">
                                  <div class="input-field col m3 l3 s12">
                                    <input id="city" name="city" type="text" class="border-full-radius medium-height box-shadow-none" placeholder="" />
                                    <label for="city" class="floating-border-lbl">Net Payble Amount</label>
                                  </div>
                                
                              </div> <!-- End of row-->
                              <div class="row">
                                    <div class="input-field col s12">
                                      <button class="btn waves-effect waves-light gradient-45deg-red-pink z-depth-4 right" type="button" name="action">Save
                                        <i class="material-icons right">send</i>
                                      </button>
                                    </div>
                              </div>

                        </div>
                        
                      </div>
                  </div> <!-- End of Add Software Payment Process -->

                </form>
              </div><!-- End of row-->

            </div> <!-- End of section-->
          </div> <!-- End of container-->
        </div>


    </div><!-- End Of Main Row -->